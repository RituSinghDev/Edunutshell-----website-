...
void main() {
  ...
  if (iFrame < 1) {
    ...
    // Always apply mouse interaction
...
function FluidSimulation() {
  ...
  useFrame(({ gl, clock }) => {
    ...
    animationPhase.current = time;
    
    // Debug: log every 60 frames (~1 second)
    if (frameCount % 60 === 0) {
      console.log('Fluid animating:', { time, frameCount, isMouseActive: isMouseActive.current });
    }
    ...
    if (!isMouseActive.current) {
      // Auto-animate with continuous circular motion - larger, faster movement
      const speed1 = 1.2;
      const speed2 = 0.9;
      currentX = centerX + Math.cos(animationPhase.current * speed1) * radius;
      currentY = centerY + Math.sin(animationPhase.current * speed2) * radius;
      
      // Calculate previous position for velocity - larger time step for more visible movement
      const dt = 0.05;
      prevX = centerX + Math.cos((animationPhase.current - dt) * speed1) * radius;
      prevY = centerY + Math.sin((animationPhase.current - dt) * speed2) * radius;
    } else {
      // Use actual mouse position
      currentX = mouse.current.x;
      currentY = mouse.current.y;
      prevX = mouse.current.prevX;
      prevY = mouse.current.prevY;
    }
